...
def run_ai_factory(google_key, pet_info, temp_file_path, story_mode, genre, progress_bar, status_text):
    ...
    try:
        ...
        tone_instruction = (
            f"一律の「〜なの！」口調ではなく、登録されたペットの性別（{gender_display}）、性格（{pet_info.get('personality', '元気')}、詳細: {pet_info.get('personality_detail', '')}）や年齢（{pet_info.get('age_display', '不明')}）などを総合的に加味し、"
            f"さらに画像から分析したその瞬間の心情に一番マッチする、自然で最高に愛らしいその子自身のオリジナルの口調（「〜だよ」「〜だね」など、不自然に偏らない表現）にしてください。"
            f"特に性別が「女の子」の場合は、一人称のセリフや心の声を絶対に「僕」「おれ」等の男の子らしい言葉にせず、「わたし」「あたし」等にし、女の子らしい自然で優しく可愛いトーンにしてください。"
        )
        ...
        if story_mode == "絵本小説風（客観的な視点から）":
            ...
            - 最初から最後まで100% {pet_info['name']}ちゃん自身の一人称（私/僕）だけで語り尽くさせ、{pet_info['owner_call']}に向かって胸の奥の溢れんばかりの本音をたくさん伝える文章にしてください。
            - 登録されているペット情報、性格、エピソード、および写真から解析した「ペットの気持ち」を最大限に加味し、その子自身の本当の魂が宿ったかのような最高品質のストーリーにしてください。
            - 【最重要ルール】: 「【起】」「【承】」「【転】」「【結】」や「シーン1」といった見出し・構成記号・数字は【絶対に一切出力しないでください】。
            - 記号や見出しを使わず、自然な段落分けのみを用いて、最初から最後まで{pet_info['name']}ちゃんの一人称の美しい独白ストーリーとして執筆してください。
            """

        prompt = f"""
        あなたはペットの何気ない日常のできごとから家族への溢れる愛情を翻訳する「世界最高峰の動物文学作家」であり、画像生成AI（DALL-E 3）の動作原理を熟知した「天才プロンプトエンジニア」です。
        提供されたメディア（画像）と登録されたペット情報をもとに、以下の3つのコンテンツを極めてエモーショナルかつ最高品質で生成してください。

        【登録されているペットの基本情報】
        ・名前: {pet_info['name']}
        ・性別: {pet_info.get('gender', '女の子')}
        ...
        1. ペットの性別（{pet_info.get('gender', '女の子')}）の完全な反映：
           - 今回のペットは「{pet_info.get('gender', '女の子')}」です。一人称の心の声やセリフでは、絶対に「僕」「おれ」などの男の子風の表現を使用しないでください。女の子であることに調和する「わたし」「あたし」等の自然で女の子らしい言葉遣いを徹底してください。
        ...
        1. ペットの気持ち分析 (feelings):
           - 提供された画像から読み取れる、{pet_info['name']}ちゃんのその瞬間の本当の気持ちや感情、{pet_info['owner_call']}へのまっすぐな愛着を、温かい言葉で語りかけるように執筆してください（日本語、150〜200文字程度。圧倒的な文芸クオリティで）。
